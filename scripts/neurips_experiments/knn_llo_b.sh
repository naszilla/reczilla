#!/bin/bash
# run all KNN algs with the leave-last-out dataset splits (llo)
...
experiment_base=neurips-knn

# maximum number of experiments (background processes) that can be running
MAX_PROCESSES=10

# params
# only use 30 samples for KNN - because we're using each similarity metric
alg_seed=0
num_samples=30
param_seed=3
