...
def split_data_on_global_timestamp(URM_all, URM_timestamp, test_ts_percentile=10):

    URM_all = sps.csr_matrix(URM_all)
    URM_timestamp = sps.csr_matrix(URM_timestamp)
    
    percentiles_to_compute = [100 - 2 * test_ts_percentile, 100 - test_ts_percentile]

    ts_eval, ts_test = np.percentile(sorted(URM_timestamp.data), percentiles_to_compute, interpolation='lower')
    ...
    for user_index in range(URM_all.shape[0]):
        ...
        if len(user_profile) >= 3:
            ...
            val_pos = np.where((user_sequence >= ts_eval) & (user_sequence < ts_test), True, False)
            train_pos = np.where(user_sequence < ts_eval, True, False)
    ...
    print(f"split_data_on_global_timestamp: {len(skipped_users)} cold users of total {URM_all.shape[0]} users skipped")

    return URM_train, URM_validation, URM_test, skipped_users
